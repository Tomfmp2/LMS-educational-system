export function renderFavoritos() {
  ...
  container.id = 'section-favoritos';
  ...
  container.innerHTML = `
    ...
    <main>
      ...
      <p id="mensaje-vacio" style="display:none;">
        Aún no has marcado ningún curso como favorito.<br>
        Ve a <strong>Cursos</strong> y agrega tus favoritos.
      </p>
    </main>
  `;
  ...
  if (!sesion) {
    container.querySelector('#mensaje-vacio').textContent = 'Debes iniciar sesión para ver tus favoritos.';
  ...
  const contenedor = container.querySelector('#contenedor-favoritos');
  ...
  if (favoritos.length === 0) {
    ...
    favoritos.forEach(curso => {
      ...
      card.innerHTML = `
        ...
        <img src="${curso.imagen || 'https://images.pexels.com/photos/4195325/pexels-photo-4195325.jpeg?auto=compress&cs=tinysrgb&w=600&h=400&fit=crop'}" alt="${curso.nombre}" onerror="this.src='https://images.pexels.com/photos/4195325/pexels-photo-4195325.jpeg?auto=compress&cs=tinysrgb&w=600&h=400&fit=crop'">
        <div class="curso-info">
          <h3>${curso.nombre}</h3>
          <p>${curso.descripcion}</p>
          <span class="profesor">Docente: ${nombreDocente}</span>
        </div>
        <button class="btn-eliminar" data-codigo="${curso.codigo}">Remover</button>
      `;
    ...
    // Agregar event listeners para remover de favoritos
    contenedor.querySelectorAll('.btn-eliminar').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation(); // Evitar que se abra el detalle del curso
        
        const codigoCurso = btn.dataset.codigo;
        const curso = favoritos.find(f => f.codigo === codigoCurso);
        
        if (curso) {
          // Eliminar directamente
          const nuevosFavoritos = favoritos.filter(f => f.codigo !== codigoCurso);
          ...
          notificacionExito(`"${curso.nombre}" removido de favoritos`);
        }
      });
    });
  }
